...
const AuthLayout = ({ children, title, subtitle, badge = "DelishDrop Access" }) => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900/20 to-slate-950 p-4 md:p-8 relative overflow-hidden">
      {/* Animated background orbs */}
      <div className="absolute top-0 -left-40 w-80 h-80 bg-slate-600 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob"></div>
      <div className="absolute top-0 -right-40 w-80 h-80 bg-slate-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000"></div>
      <div className="absolute -bottom-8 left-20 w-80 h-80 bg-slate-600 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-4000"></div>
      ...
      <div className="mx-auto grid h-[calc(100vh-2rem)] max-h-[920px] w-full max-w-7xl overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl md:h-[calc(100vh-4rem)] md:grid-cols-2">
        ...
        <section className="relative hidden overflow-hidden p-12 text-white md:flex md:flex-col md:justify-between">
          ...
          <div className="relative z-10 space-y-6">
            <div className="inline-flex items-center gap-2 rounded-full bg-slate-700/40 border border-slate-500/30 px-4 py-2 text-sm font-semibold backdrop-blur-md">
              <span className="inline-flex h-2.5 w-2.5 rounded-full bg-slate-400 animate-pulse" />
              {badge}
            </div>
            ...
            <div>
              <h1 className="text-5xl md:text-6xl font-black leading-tight">
                ...
                <span className="block text-slate-400">Made Simple</span>
              </h1>
